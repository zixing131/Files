...
| Closed-tab recovery | Passed full-state runtime round trips | Localized tab context and native File menus close one/all other tabs and reopen up to 20 recently closed tabs with Command-Shift-T or Control-Shift-T; primary/secondary paths, active pane, split ratio, grid/details mode and sorting restore together, and Close Other Tabs reopens the nearest right-hand tab first |
...
- Tab context and native File menus close a tab, close all other tabs, or reopen the latest of 20 closed tabs with its complete dual-pane state; Command-Shift-T and Control-Shift-T provide the conventional recovery shortcut.
