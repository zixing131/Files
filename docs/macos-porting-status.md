...
| Keyboard and assistive access | Passed runtime focus/label checks | F6 and Shift-F6 cycle forward/backward through the tab strip, sidebar, breadcrumb, search, command toolbar, active file pane, optional preview and view controls. Localized automation names cover these regions, icon-only controls, toolbar commands, tab headers, sidebar entries and realized grid/details items in both panes; a hands-on VoiceOver announcement/order pass remains required |
...
- F6/Shift-F6 cycles the major interface regions in either direction, while localized accessibility names identify tabs, locations, navigation/search, commands and every realized file item to macOS assistive technologies.
