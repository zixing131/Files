...
| NTFS ACL/安全属性 | POSIX mode、ACL、owner/group、扩展属性；UI 重新设计 | 后续 |
| Windows 文件标签 | Finder tags/扩展属性；先验证公开 API 与 App Store 合规性 | 后续 |
| `.lnk` 快捷方式 | macOS alias、symbolic link；不保证与 Windows 快捷方式等价 | 后续 |
...
- Finder tags、扩展属性、POSIX ACL/owner/group 属性页。
