...
- macOS 菜单栏、标准快捷键、服务菜单、窗口恢复和原生对话框。独立多窗口、活动窗口菜单路由、并发安全持久化、所有窗口工作区及 AppKit 尺寸/位置/最大化状态的跨启动恢复已完成，并会在显示器变化后约束回可见区域；透明全内容标题栏已将 Files 标签栏与原生红黄绿按钮合并到同一顶部区域。标签页右键/原生菜单、关闭其他标签页和 Command-Shift-T 完整状态恢复也已接入中英文界面。
