...
| Native macOS menus, windows and shortcuts | Passed build/runtime checks | Localized application, File, Edit, View, Go and Window menus route through the active window with live validation snapshots; New Window and Close Window use conventional Command shortcuts, secondary windows start independently without overwriting the primary restored workspace, and 16 Command/Command-Shift/Command-Option accelerators coexist with the portable Control bindings |
...
- Independent macOS windows through File > New Window / Command-N, with Command-Shift-W close behavior, active-window menu routing, retained window lifetimes and workspace-safe settings persistence.
